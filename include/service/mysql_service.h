...
struct Activity {
    ...
    int         stock;
    int64_t     start_time;  // 毫秒时间戳
    int64_t     end_time;    // 毫秒时间戳
    int8_t      status;       // 0=未启动/已停止, 1=进行中
};
...
public:
    ...
    bool decrease_product_stock(const std::string& product_id, int amount);  // 新增：原子扣除库存
    bool add_product_stock(const std::string& product_id, int amount);       // 新增：原子增加库存
    ...
    // 活动操作
    bool create_activity(const Activity& act);
    std::optional<Activity> get_activity_by_product(const std::string& product_id);
    bool update_activity_status(const std::string& id, int8_t status);
    int count_orders_by_product(const std::string& product_id);
    std::vector<Activity> get_all_active_activities();
